...
if [ "$user_password" = "$correct_password" ]; then
    ...
    case $choice in
        1)
            echo "正在安装全部服务"
            wget -O docker-compose-1.yml --no-check-certificate https://gh.llkk.cc/https://raw.githubusercontent.com/ATaKi-Myt/gitdemo/refs/heads/main/docker-compose.yml
