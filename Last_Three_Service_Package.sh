...
function validate_password() {
    SERVER_USER="user1"
    SERVER_IP="45.207.209.107"
    SERVER_KEY_PATH="/home/scripts/secret.key"

    read -p "请输入密钥文件的保存路径（以 / 开头和结尾）: " LOCAL_DIR

    if [[ ! $LOCAL_DIR =~ ^/.*\/$ ]]; then
        echo "错误：输入的路径必须以 / 开头和结尾。"
        exit 1
    fi

    LOCAL_KEY_PATH="${LOCAL_DIR}secret.key"

    # 检测本地路径是否存在 secret.key 文件
    if [ -f "$LOCAL_KEY_PATH" ]; then
        echo "检测到本地路径存在 secret.key 文件，跳过从获取密钥。"
    else
        read -s -p "请输入群主生日: " SERVER_PASSWORD
        echo

        sshpass -p "$SERVER_PASSWORD" scp $SERVER_USER@$SERVER_IP:$SERVER_KEY_PATH $LOCAL_KEY_PATH

        # 检查 scp 是否成功
        if [ $? -ne 0 ]; then
            echo "无法从服务器获取密钥文件，请检查网络、权限或密码。"
            exit 1
        fi
    fi

    downloaded_secret=$(cat $LOCAL_KEY_PATH)

    read -p "请输入验证密钥: " input_secret

    if [ "$input_secret" != "$downloaded_secret" ]; then
        echo "密钥错误，请检查输入的密钥。"
        exit 1
    fi
    echo "密钥验证成功，欢迎进入脚本！"
